...
class IcyParser(object):
    ...
    def get_icy_information(self, url):
        ...
        t1 = threading.Thread(target=self.icy_information)
        t1.start()

    def icy_information(self):
        ...
        while self.thread_should_be_running:
            ...
            if length_specifier != 0:
                ...
                self.icy_streamtitle = re.findall("(?<=StreamTitle=').*(?=';)", streamtitle)[0]
                print("{0} {1}".format(self.icy_streamtitle, time.strftime("%d-%m-%Y %H:%M:%S")))
                self.icystream.write("{} {}".format(time.strftime("%d-%m-%Y %H:%M:%S"),
                                                    self.icy_streamtitle + "\n"))
                self.icystream.flush()
            else:
                print("No StreamTitle given")
